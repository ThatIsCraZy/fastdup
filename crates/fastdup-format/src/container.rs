...
impl SealedContainer {
    ...
    pub fn encode(
        ...
        let layout = raw_container_layout(chunks)?;
        ...
        for chunk in chunks {
            let encoded = RawRecord::encode(chunk)?;
            let entry = IndexEntry::from_encoded_record(&encoded, record_offset);
        ...
        container[footer_offset_usize + FOOTER_HASH_OFFSET
            ..footer_offset_usize + FOOTER_HASH_OFFSET + 32]
    ...
    /// Returns the fully verified RAW records in their physical order.
    ///
    /// Callers may use this bounded view to rebuild location indexes. The
    /// records remain owned by this already validated immutable container.
    #[must_use]
...
impl IndexEntry {
    fn from_encoded_record(bytes: &[u8], record_offset: u64) -> Self {
        let mut chunk_id = [0_u8; 32];
        chunk_id.copy_from_slice(&bytes[128..160]);
        Self {
            chunk_id: ChunkId(chunk_id),
            logical_length: get_u32(bytes, 164),
            record_offset,
    ...
    fn encode(&self, output: &mut [u8]) {
        ...
        put_u32(output, 52, 0);
        put_u16(output, 56, RAW_CODEC);
        put_u16(output, 58, 0);
        put_u32(output, 60, self.record_crc32c);
        put_u32(output, 96, self.logical_length);
        put_u32(output, 100, self.logical_length);
    }
    ...
    fn decode(bytes: &[u8]) -> Result<Self, FormatError> {
        if bytes.len() != INDEX_ENTRY_BYTES_USIZE
            || get_u32(bytes, 36) != 0
            || get_u32(bytes, 52) != 0
            || get_u16(bytes, 56) != RAW_CODEC
            || get_u16(bytes, 58) != 0
            || bytes[64..96].iter().any(|byte| *byte != 0)
            || get_u32(bytes, 96) != get_u32(bytes, 32)
            || get_u32(bytes, 100) != get_u32(bytes, 32)
...
fn raw_container_layout(chunks: &[&[u8]]) -> Result<ContainerLayout, FormatError> {
    if chunks.is_empty() {
        return Err(FormatError::InvalidContainerLayout);
    }
    let record_count = u32::try_from(chunks.len()).map_err(|_| FormatError::ArithmeticOverflow)?;
    let mut index_offset =
        u64::try_from(HEADER_BYTES).map_err(|_| FormatError::ArithmeticOverflow)?;
    for chunk in chunks {
        let record_length = raw_record_length(chunk.len())?;
        index_offset = index_offset
            .checked_add(u64::try_from(record_length).map_err(|_| FormatError::ArithmeticOverflow)?)
            .ok_or(FormatError::ArithmeticOverflow)?;
    }
    ...
    let layout = ContainerLayout {
        record_count,
        chunk_entry_count: record_count,
        index_offset,
        index_length,
        footer_offset,
        file_length,
