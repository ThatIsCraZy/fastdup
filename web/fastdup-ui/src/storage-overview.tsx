 ...
 return <>
  ...
  <Card><CardHeader><h2>{t("Logisch vs. physisch")}</h2></CardHeader><CardContent>
   <div className="storage-comparison">{([{label:"Logische Belegung",amount:logical,Icon:FileStack},{label:"Physische Belegung",amount:physical,Icon:Database}]).map(({label, amount, Icon}) => {
    return <div className="storage-measure" key={label}><Icon size={20} aria-hidden="true"/><Meter size="large" label={label} value={amount} max={amount == null ? null : scale} valueText={bytes(amount)}/></div>;
   })}</div>
  ...
  <div className="storage-volumes">{volumes.map(volume => {
   ...
   return <Card key={volume.role}><CardHeader><div><h2>{volume.role}</h2><p>{t(volume.description)}</p></div></CardHeader><CardContent>
    <Meter size="large" label="Belegt" ariaLabel={`${volume.role} ${t("Belegung")}`} value={volume.used} max={volume.capacity}
     valueText={bytes(volume.used)} limitLabel="belegt von" limitText={volume.capacity == null ? undefined : bytes(volume.capacity)}/>
