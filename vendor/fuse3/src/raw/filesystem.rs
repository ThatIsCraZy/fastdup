...
pub trait Filesystem {
    ...
    /// Write data while transferring ownership of the request payload.
    ///
    /// The session calls this method because it already owns the bytes needed
    /// by the spawned request task. Implementations that retain write data can
    /// override it and avoid another full payload copy. The default preserves
    /// the borrowed [`Self::write`] interface for existing filesystems.
    #[allow(clippy::too_many_arguments)]
