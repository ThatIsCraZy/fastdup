...
artifact_root="${FASTDUP_ARTIFACT_ROOT:-$workspace/.artifacts}"
package_root="$artifact_root/package"
rpmbuild_root="$artifact_root/rpmbuild"
stage_root="$package_root/fastdup-0.7.0"
...
tar -C "$package_root" -czf \
    "$rpmbuild_root/SOURCES/fastdup-0.7.0-x86_64.tar.gz" \
    fastdup-0.7.0
...
find "$rpmbuild_root/RPMS" -type f -name 'fastdup-0.7.0-*.x86_64.rpm' -print
